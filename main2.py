...
def create_empty_extraction_payload():
    """Create a default structure for extracted PDF data."""
    return {
        'invoice_number': None,
        'invoice_date': None,
        'supplier_name': None,
...
def extract_entity_tables(pdf_file_path: str) -> Optional[Dict]:
    """
    Attempt to extract structured tabular data for product, supply, or customer PDFs.
    Returns a populated extraction payload when a supported table is detected.
    """
    entity_column_rules = {
        'product': {
            'product_id': ['product id', 'product code', 'sku', 'item id'],
            'product_name': ['product name', 'item name', 'description'],
            'category': ['category', 'type', 'class'],
            'price': ['price', 'unit price', 'cost', 'amount'],
            'stock': ['stock', 'qty', 'quantity', 'inventory'],
            'supply_id': ['supply id', 'supplier id', 'vendor id']
        },
        'supply': {
            'supply_id': ['supply id', 'supplier id', 'vendor id', 'supply code'],
            'supplier_name': ['supplier name', 'vendor name', 'company name'],
    ...
    required_matches = {'product': 4, 'supply': 3, 'customer': 3}

    def map_headers(headers: List[str], doc_type: str) -> Dict[str, str]:
        header_map = {}
        rules = entity_column_rules[doc_type]
        ...
        with pdfplumber.open(pdf_file_path) as pdf:
            for page in pdf.pages:
                ...
                for table in tables:
                    ...
                    for doc_type in ['product', 'supply', 'customer']:
                        header_map = map_headers(headers, doc_type)
                        if len(header_map) >= required_matches[doc_type]:
                            ...
                            for _, row in df_data.iterrows():
                                ...
                                for field, header_name in header_map.items():
                                    ...
                                    if pd.isna(value):
                                        record[field] = None
                                        continue
                                    if field in numeric_fields[doc_type]:
                                        cleaned = re.sub(r'[^\d\.-]+', '', str(value))
                                        try:
                                            record[field] = float(cleaned) if cleaned not in ["", "-", "."] else None
                                        except ValueError:
                                            record[field] = None
                                    else:
                                        record[field] = str(value).strip()
                                if any(val not in (None, '', 'null') for val in record.values()):
                                    collected_records.append(record)
                            break
                    if detected_type:
                        # Continue parsing other tables but no need for invoice extraction
                        continue

        if detected_type and collected_records:
            payload = create_empty_extraction_payload()
            payload['document_type'] = detected_type
            payload['records'] = collected_records
            payload['raw_text'] = raw_text
            return payload
    except Exception as e:
        print(f"Entity table extraction failed: {e}")

    return None

def get_default_db_creds():
    """Get default database credentials from environment variables with fallback defaults"""
    return {
        'user': os.getenv('DB_USER', 'root'),
        'password': os.getenv('DB_PASSWORD', '8457'),
        'host': os.getenv('DB_HOST', 'localhost'),
        'database': os.getenv('DB_NAME', 'retail')
    }

def get_db_uri():
    """Get database connection URI from credentials"""
    creds = get_default_db_creds()
    return f"mysql+pymysql://{creds['user']}:{creds['password']}@{creds['host']}/{creds['database']}"
...
if "db" not in st.session_state:
    try:
        ...
        st.error("Please check your database credentials in the .env file or environment variables.")
        st.stop()
...
class AdvancedSQLOptimizer:
    ...
    def _build_table_relationships(self) -> Dict:
        ...
        return {
            'supply': {
                ...
                'related_tables': {
                    'product': {
                        'join_condition': 'sp.supply_id = pr.supply_id',
                        'join_type': 'LEFT JOIN'
                    }
                }
            },
            'product': {
                ...
                'related_tables': {
                    'supply': {
                        'join_condition': 'pr.supply_id = sp.supply_id',
    ...
    def _build_column_mapping(self) -> Dict:
        ...
        return {
            ...
            'supply': 'supply.supplier_name',
            'supply_id': 'supply.supply_id',
            'supplier_id': 'supply.supply_id',
            'supplier_phone': 'supply.phone',
            'supplier_email': 'supply.email',
            'supplier_city': 'supply.city',
            'vendor': 'supply.supplier_name',
            ...
            'product_supply_id': 'product.supply_id',
    ...
    def _load_advanced_patterns(self) -> Dict:
        ...
        return {
            'count_queries': [
                r'\b(?:how many|count|total number|number of)\b.*\b(?:suppliers?|products?|customers?)\b',
                r'\bcount\s+(?:of\s+)?(?:suppliers?|products?|customers?)\b'
            ],
            'show_all_queries': [
                r'\b(?:show|list|display|get)\s+(?:all|every)\s+(?:suppliers?|products?|customers?)\b',
                r'\ball\s+(?:suppliers?|products?|customers?)\b'
            ],
            'join_queries': [
                r'\bproducts?\s+(?:from|by)\s+(?:supplier|vendor)\b',
                r'\bsuppliers?\s+(?:providing|selling)\b',
                r'\bcustomers?\s+(?:buying|interested in)\b'
            ],
            'aggregation_queries': [
                r'\b(?:total|sum|average|avg|max|maximum|min|minimum)\s+(?:price|stock|inventory|value)\b',
                r'\b(?:group by|grouped by|per|by)\s+(?:supplier|category|city|customer)\b'
            ],
            'comparison_queries': [
                r'\b(?:top|highest|largest|biggest)\s+\d*\s*(?:products?|suppliers?)\b',
                r'\b(?:bottom|lowest|smallest)\s+\d*\s*(?:products?|suppliers?)\b'
            ],
            'specific_lookups': [
                r'\b(?:supplier|vendor)\s+(?:id|code|number|name)\s+([A-Z0-9-\s]+)\b',
                r'\bproduct\s+(?:id|code|number|name)\s+([A-Z0-9-\s]+)\b',
                r'\bcustomer\s+(?:id|name)\s+([A-Za-z\s]+)\b'
            ]
        }
    ...
    def _extract_intent(self, query: str, analysis: QueryAnalysis):
        ...
        intent_patterns = {
            ...
            'list_products': r'\b(?:list|show|get)\s+(?:all\s+)?products?\b',
            'list_customers': r'\b(?:list|show|get)\s+(?:all\s+)?customers?\b',
            'product_by_supplier': r'\bproducts?\s+(?:from|by)\s+(?:supplier|vendor)\b',
            'top_analysis': r'\b(?:top|highest|largest)\s+\d*\s*(?:products?|suppliers?)\b',
            'price_analysis': r'\b(?:total|sum|average|avg|maximum|max|minimum|min)\s+(?:price|cost|value|stock|inventory)\b',
            'supplier_lookup': r'\bsupplier\s+(?:id|code|name)\s+([A-Z0-9-\s]+)\b',
            'product_lookup': r'\bproduct\s+(?:id|code|name)\s+([A-Z0-9-\s]+)\b',
            'customer_lookup': r'\bcustomer\s+(?:id|name)\s+([A-Za-z\s]+)\b',
            'count_entities': r'\b(?:count|number of)\s+(?:suppliers|products|customers|items)\b'
        }
    ...
    def _analyze_table_requirements(self, query: str, analysis: QueryAnalysis):
        ...
        table_keywords = {
            'supply': [
                'supplier', 'suppliers', 'vendor', 'supply', 'supply_id',
                'supplier name', 'supplier phone', 'supplier email', 'supplier city'
            ],
            'product': [
                'product', 'products', 'item', 'catalog', 'category',
                'price', 'stock', 'inventory', 'product id', 'product name'
            ],
            'customer': [
        ...
        # Default to product if no specific table detected and it's not a general count
        if not analysis.tables_needed and not any(k in query for k in ['count', 'number of']):
            analysis.tables_needed.add('product')
    ...
    def _extract_filters_advanced(self, query: str, analysis: QueryAnalysis):
        ...
        supplier_match = re.search(r'\b(?:supplier|vendor)\s+(?:id|code|number)\s+([A-Z0-9-]+)\b', query, re.IGNORECASE)
        if supplier_match:
            analysis.filters['supply_id'] = supplier_match.group(1).strip()
            analysis.tables_needed.add('supply')

        supplier_name_match = re.search(r'\b(?:supplier|vendor)\s+(?:name)\s+([A-Za-z0-9\s]+)\b', query, re.IGNORECASE)
        if supplier_name_match:
            analysis.filters['supplier_name'] = supplier_name_match.group(1).strip()
            analysis.tables_needed.add('supply')

        product_match = re.search(r'\bproduct\s+(?:id|code|number)\s+([A-Z0-9-]+)\b', query, re.IGNORECASE)
        if product_match:
            analysis.filters['product_id'] = product_match.group(1).strip()
            analysis.tables_needed.add('product')

        product_name_match = re.search(r'\bproduct\s+(?:name)\s+([A-Za-z0-9\s]+)\b', query, re.IGNORECASE)
        if product_name_match:
            analysis.filters['product_name'] = product_name_match.group(1).strip()
            analysis.tables_needed.add('product')

        category_match = re.search(r'\bcategory\s+([A-Za-z0-9\s]+)\b', query, re.IGNORECASE)
        if category_match:
            analysis.filters['category'] = category_match.group(1).strip()
            analysis.tables_needed.add('product')

        customer_match = re.search(r'\bcustomer\s+(?:id|number)\s+([A-Z0-9-]+)\b', query, re.IGNORECASE)
        if customer_match:
            analysis.filters['customer_id'] = customer_match.group(1).strip()
            analysis.tables_needed.add('customer')

        customer_name_match = re.search(r'\bcustomer\s+(?:name)\s+([A-Za-z\s]+)\b', query, re.IGNORECASE)
        if customer_name_match:
            analysis.filters['customer_name'] = customer_name_match.group(1).strip()
            analysis.tables_needed.add('customer')

        city_match = re.search(r'\b(?:in|from)\s+([A-Za-z\s]+)\s+city\b', query, re.IGNORECASE)
        if city_match:
            analysis.filters['city'] = city_match.group(1).strip()

        # Price filters (for product.price)
        price_patterns = {
        ...
        for filter_type, pattern in price_patterns.items():
            match = re.search(pattern, query, re.IGNORECASE)
            if match:
                analysis.filters['price_filter'] = {
                    'type': filter_type,
    ...
    def _detect_aggregations(self, query: str, analysis: QueryAnalysis):
        ...
        agg_patterns = {
            ...
            'SUM': r'\b(?:total|sum)\s+(?:price|value|stock|inventory)\b',
            'AVG': r'\b(?:average|avg|mean)\s+(?:price|value|stock|inventory)\b',
            'MAX': r'\b(?:maximum|max|highest|largest)\s+(?:price|value|stock|inventory)\b',
            'MIN': r'\b(?:minimum|min|lowest|smallest)\s+(?:price|value|stock|inventory)\b'
        }
        ...
        for agg_func, pattern in agg_patterns.items():
            if re.search(pattern, query, re.IGNORECASE):
                ...
                if agg_func in ['SUM', 'AVG', 'MAX', 'MIN']:
                    ...
                    if re.search(r'\b(?:suppliers|vendors)\b', query, re.IGNORECASE):
                        analysis.tables_needed.add('supply')
                    elif re.search(r'\b(?:customers|clients)\b', query, re.IGNORECASE):
                        analysis.tables_needed.add('customer')
                    else:
                        analysis.tables_needed.add('product')
    ...
    def _detect_grouping_ordering(self, query: str, analysis: QueryAnalysis):
        ...
        # GROUP BY patterns
        group_patterns = [
            r'\bper\s+(supplier|customer|category|city)\b',
            r'\bby\s+(supplier|customer|category|city)\b',
            r'\bgroup(?:ed)?\s+by\s+(supplier|customer|category|city)\b'
        ]
        ...
        for pattern in group_patterns:
            ...
            if match:
                ...
                elif group_field_natural == 'category':
                    analysis.grouping.append('product.category')
                    analysis.tables_needed.add('product')
            ...
            if re.search(pattern, query, re.IGNORECASE):
                ...
                if 'top' in query or 'bottom' in query:
                    if 'price' in query or 'value' in query:
                        analysis.specific_columns.append('price')
                    elif 'stock' in query or 'inventory' in query:
                        analysis.specific_columns.append('stock')
    ...
    def generate_optimized_sql(self, user_query: str, chat_history: str = "",
        ...
        if is_invoice_specific and invoice_id:
            analysis.filters['product_id'] = invoice_id
            analysis.tables_needed.add('product')
    ...
    def _generate_simple_sql(self, analysis: QueryAnalysis, user_query: str = "", invoice_id: str = None) -> str:
        ...
        table = list(analysis.tables_needed)[0] if analysis.tables_needed else 'product'

        select_clause_parts = []
        if analysis.aggregations:
            if 'COUNT' in analysis.aggregations:
                select_clause_parts.append("COUNT(*) as total_count")
            elif table == 'product':
                select_clause_parts.append(f"{analysis.aggregations[0]}(price) as result")
            elif table == 'supply':
                select_clause_parts.append("COUNT(*) as total_suppliers")
            else:
                select_clause_parts.append("COUNT(*) as total_count")
        elif analysis.specific_columns:
            for col in analysis.specific_columns:
                mapped = self.column_mapping.get(col, col)
                select_clause_parts.append(mapped.split('.', 1)[-1] if '.' in mapped else mapped)
        else:
        ...
        for filter_key, filter_value in analysis.filters.items():
            if table == 'supply' and filter_key in ['supply_id', 'supplier_name', 'city']:
                column = 'supply_id' if filter_key == 'supply_id' else ('supplier_name' if filter_key == 'supplier_name' else 'city')
                if filter_key == 'supplier_name':
                    where_conditions.append(f"{column} LIKE '%{filter_value}%'")
                else:
                    where_conditions.append(f"{column} = '{filter_value}'")
            elif table == 'product' and filter_key in ['product_id', 'product_name', 'category', 'supply_id']:
                column = {
                ...
                if filter_key in ['product_name', 'category']:
                    where_conditions.append(f"{column} LIKE '%{filter_value}%'")
                else:
                    where_conditions.append(f"{column} = '{filter_value}'")
            elif table == 'customer' and filter_key in ['customer_id', 'customer_name', 'city']:
                column = 'customer_id' if filter_key == 'customer_id' else ('first_name' if filter_key == 'customer_name' else 'city')
                if filter_key == 'customer_name':
                    where_conditions.append(f"{column} LIKE '%{filter_value}%'")
                else:
                    where_conditions.append(f"{column} = '{filter_value}'")
            elif table == 'product' and filter_key == 'price_filter':
                price_condition = self._build_price_condition(filter_value)
                if price_condition:
                    where_conditions.append(price_condition.replace('pr.', ''))
        ...
        if analysis.grouping:
            group_clause = f" GROUP BY {', '.join([g.split('.',1)[-1] for g in analysis.grouping])}"

        order_clause = ""
        if analysis.ordering and select_clause_parts:
            order_field = select_clause_parts[0].split(' as ')[-1]
    ...
    def _generate_template_sql(self, analysis: QueryAnalysis, invoice_id: str = None) -> str:
        ...
        alias_map = {'supply': 'sp', 'product': 'pr', 'customer': 'cu'}

        # Determine main table and alias
        main_table_name = list(analysis.tables_needed)[0] if analysis.tables_needed else 'product'
        main_table_alias = alias_map.get(main_table_name, main_table_name[:2])
        ...
        if analysis.intent == 'list_suppliers':
            analysis.tables_needed.add('supply')
            main_table_name = 'supply'
            main_table_alias = 'sp'
            from_clause = f"FROM supply {main_table_alias}"
            select_parts = ["sp.supply_id", "sp.supplier_name", "sp.phone", "sp.email", "sp.city"]
        elif analysis.intent == 'list_products':
            analysis.tables_needed.add('product')
            main_table_name = 'product'
            main_table_alias = 'pr'
            from_clause = f"FROM product {main_table_alias}"
            select_parts = ["pr.product_id", "pr.product_name", "pr.category", "pr.price", "pr.stock", "pr.supply_id"]
        elif analysis.intent == 'list_customers':
            analysis.tables_needed.add('customer')
            main_table_name = 'customer'
            main_table_alias = 'cu'
            from_clause = f"FROM customer {main_table_alias}"
            select_parts = ["cu.customer_id", "cu.first_name", "cu.last_name", "cu.email", "cu.phone", "cu.city"]
        elif analysis.intent == 'product_by_supplier':
            analysis.tables_needed.update({'product', 'supply'})
            main_table_name = 'product'
            main_table_alias = 'pr'
            from_clause = f"FROM product {main_table_alias}"
            select_parts = ["pr.product_id", "pr.product_name", "pr.category", "pr.price", "pr.stock", "sp.supplier_name", "sp.city"]
        elif analysis.aggregations:
            agg_func = analysis.aggregations[0]
            if agg_func == 'COUNT':
                select_parts = ["COUNT(*) as total_count"]
            else:
                target_column = 'pr.price' if main_table_name == 'product' else f"{main_table_alias}.price"
                select_parts = [f"{agg_func}({target_column}) as result"]
        elif analysis.specific_columns:
            for col_term in analysis.specific_columns:
                ...
                if mapped_col:
                    ...
                    select_parts.append(col_term)
        else:
            select_parts.append(f"{main_table_alias}.*")
        ...
        for j_plan in analysis.joins_required:
            joined_alias = alias_map.get(j_plan['table'], j_plan['table'][:2])
            join_clauses.append(f"{j_plan['type']} {j_plan['table']} {joined_alias} ON {j_plan['condition']}")

        if 'product' in analysis.tables_needed and 'supply' in analysis.tables_needed and \
           not any('supply' in j_clause for j_clause in join_clauses):
            join_clauses.append("LEFT JOIN supply sp ON pr.supply_id = sp.supply_id")

        # WHERE clause
        where_conditions = []
        for filter_key, filter_value in analysis.filters.items():
            if filter_key == 'supply_id':
                alias = 'sp' if 'supply' in analysis.tables_needed else main_table_alias
                where_conditions.append(f"{alias}.supply_id = '{filter_value}'")
            elif filter_key == 'supplier_name':
                alias = 'sp' if 'supply' in analysis.tables_needed else main_table_alias
                where_conditions.append(f"{alias}.supplier_name LIKE '%{filter_value}%'")
            elif filter_key == 'product_id':
                alias = 'pr' if 'product' in analysis.tables_needed else main_table_alias
                where_conditions.append(f"{alias}.product_id = '{filter_value}'")
            elif filter_key == 'product_name':
                alias = 'pr' if 'product' in analysis.tables_needed else main_table_alias
                where_conditions.append(f"{alias}.product_name LIKE '%{filter_value}%'")
            elif filter_key == 'category':
                alias = 'pr' if 'product' in analysis.tables_needed else main_table_alias
                where_conditions.append(f"{alias}.category LIKE '%{filter_value}%'")
            elif filter_key == 'customer_id':
                alias = 'cu' if 'customer' in analysis.tables_needed else main_table_alias
                where_conditions.append(f"{alias}.customer_id = '{filter_value}'")
            elif filter_key == 'customer_name':
                alias = 'cu' if 'customer' in analysis.tables_needed else main_table_alias
                where_conditions.append(f"{alias}.first_name LIKE '%{filter_value}%'")
            elif filter_key == 'city':
                if 'supply' in analysis.tables_needed:
                    where_conditions.append(f"sp.city LIKE '%{filter_value}%'")
                elif main_table_name == 'customer':
                    where_conditions.append(f"{main_table_alias}.city LIKE '%{filter_value}%'")
            elif filter_key == 'price_filter':
                price_condition = self._build_price_condition(filter_value)
                if price_condition:
        ...
        if analysis.grouping:
            ...
            for col_path in analysis.grouping:
                ...
                if len(parts) == 2 and parts[0] in alias_map:
                    aliased_grouping.append(f"{alias_map[parts[0]]}.{parts[1]}")
                else:
                    aliased_grouping.append(col_path)
        ...
        if analysis.ordering:
            order_field = None
            if analysis.aggregations:
                order_field = "result" if analysis.aggregations[0] != 'COUNT' else "total_count"
            elif 'price' in [self.column_mapping.get(c, c) for c in analysis.specific_columns]:
                order_field = "pr.price"
            elif 'stock' in [self.column_mapping.get(c, c) for c in analysis.specific_columns]:
                order_field = "pr.stock"
    ...
    def _build_price_condition(self, price_filter: Dict) -> str:
        """Helper to build MySQL price conditions for product.price."""
        filter_type = price_filter['type']
        values = price_filter['values']

        if filter_type == 'greater_than':
            return f"pr.price > {values[0]}"
        elif filter_type == 'less_than':
            return f"pr.price < {values[0]}"
        elif filter_type == 'between':
            return f"pr.price BETWEEN {values[0]} AND {values[1]}"
...
2.  **Aliases:** Always use aliases for tables: `sp` for `supply`, `pr` for `product`, `cu` for `customer`. **When a table has an alias, reference its columns with that alias (e.g., `pr.product_name`).**
3.  **Joins:** Construct `JOIN` clauses based on `Required Tables` and `Planned Joins`. Use `LEFT JOIN` unless an `INNER JOIN` is needed (e.g., when every product must have a supplier). Ensure join conditions connect the correct keys such as `pr.supply_id = sp.supply_id`.
4.  **Filtering (`WHERE` clause):**
    * Apply every entry from `Extracted Filters`.
    * Use `LIKE '%value%'` for partial text searches (supplier names, categories, cities, etc.).
    * Use numeric comparisons for price/stock filters (e.g., `pr.price > 100`).
5.  **Aggregations & Grouping:** When aggregations are present, include the specified grouping columns (supplier, category, city, etc.) in `GROUP BY`. Every non-aggregated selected column must appear in `GROUP BY`.
6.  **Ordering (`ORDER BY` clause):** Follow the `Ordering Criteria`. For "top/bottom N" requests, add the appropriate `LIMIT`.
7.  **Column Selection:**
    * Use `Specific Columns Requested` mapped via `COLUMN MAPPINGS`.
    * Without specific columns, prefer descriptive fields (e.g., IDs, names, category, price, stock) rather than `SELECT *`.
8.  **Avoid unnecessary subqueries.** Prefer direct joins unless a subquery is mandatory.
9.  **Return only the SQL query.** Do not include explanations or markdown fences.

EXAMPLES OF COMPLEX QUERIES FOR GUIDANCE:
-   **List each supplier with the products they provide and the product price:**
    ```sql
    SELECT sp.supplier_name, pr.product_name, pr.category, pr.price
    FROM product pr
    LEFT JOIN supply sp ON pr.supply_id = sp.supply_id
    ORDER BY sp.supplier_name, pr.product_name;
    ```
-   **Find all products in the "Electronics" category priced above $500:**
    ```sql
    SELECT pr.product_id, pr.product_name, pr.category, pr.price, pr.stock
    FROM product pr
    WHERE pr.category = 'Electronics' AND pr.price > 500
    ORDER BY pr.price DESC;
    ```
-   **Count customers per city:**
    ```sql
    SELECT cu.city, COUNT(cu.customer_id) AS customer_count
    FROM customer cu
    GROUP BY cu.city
...
-   **Average price per supplier:**
    ```sql
    SELECT sp.supplier_name, AVG(pr.price) AS avg_price
    FROM product pr
    INNER JOIN supply sp ON pr.supply_id = sp.supply_id
...
def extract_invoice_data(pdf_file_path):
    ...
    """
    extracted_data = create_empty_extraction_payload()
    raw_text = ""
...
def insert_entity_records_to_db(table_name: str, records: List[Dict]) -> Tuple[bool, str]:
    """Insert extracted product/supply/customer records into their respective tables."""
    if not records:
        return False, "No records to insert."

    valid_tables = {'product', 'supply', 'customer'}
    ...
    try:
        ...
        with engine.begin() as connection:
            df_records.to_sql(table_name, con=connection, if_exists='append', index=False)
        return True, f"Inserted {len(df_records)} rows into `{table_name}`."
    except Exception as e:
        return False, f"Failed to insert records: {e}"

# --- Main App Configuration ---
st.set_page_config(layout="wide", page_title="Retail Assistant")

# --- Sidebar ---
with st.sidebar:
    st.markdown("### Retail Assistant")

    st.subheader("Menu")
    menu_choice = st.radio(
        "",
        ["Chat", "Pdf Upload", "Login", "Chat Backup"],
        key="sidebar_menu",
    )
    ...
    if menu_choice == "Login":
        ...
        if st.button("Login", key="login_btn_sidebar"):
            if login_password != "admin123": # Simple hardcoded password for demo
                ...
                st.session_state.chat_history = [AIMessage("Hello! I'm your Retail Assistant. I'm here to help you with your invoice details.")]
...
# --- Main Content Area ---
st.title("Retail Assistant")

# Initialize session state variables if they don't exist
if "chat_history" not in st.session_state:
    st.session_state.chat_history = [
        AIMessage("Hello! I'm your Retail Assistant. Please log in using the sidebar to start chatting."),
    ]
...
if menu_choice == "Pdf Upload":
    ...
    if uploaded_file is not None:
        ...
        extracted_data = extract_entity_tables(temp_file_path)
        ...
        if extracted_data:
            ...
            doc_type = st.session_state.extracted_invoice_data.get('document_type', 'invoice')

            if doc_type in ['product', 'supply', 'customer']:
                st.subheader(f"Extracted {doc_type.title()} Records (Review and Edit)")
                records = st.session_state.extracted_invoice_data.get('records', [])
                if records:
                    ...
                    column_config_map = {
                        'product': {
                            ...
                            'supply_id': st.column_config.TextColumn("Supply ID", width="small")
                        },
                        'supply': {
                            'supply_id': st.column_config.TextColumn("Supply ID", width="small"),
                    ...
                    edited_df = st.data_editor(
                        records_df,
                        key=f"edit_{doc_type}_records",
                        num_rows="dynamic",
                        use_container_width=True,
                        column_config=column_config_map.get(doc_type, {})
                    )
                    st.session_state.extracted_invoice_data['records'] = edited_df.to_dict('records')

                    if st.button(f"Upload {doc_type.title()} Records to Database", key=f"upload_{doc_type}_records"):
                        success, message = insert_entity_records_to_db(doc_type, st.session_state.extracted_invoice_data['records'])
                        if success:
                            st.success(message)
                            st.session_state.extracted_invoice_data = None
                            st.rerun()
                    ...
                    st.warning("No structured records were detected in this PDF.")
            else:
                ...
                st.subheader("Extracted Product Details (Review and Edit)")
            ...
            with st.form("edit_invoice_form", clear_on_submit=False):
                ...
                with col_inv2:
                    st.session_state.extracted_invoice_data['invoice_date'] = st.text_input(
                        "Release Date (MM/DD/YYYY or YYYY-MM-DD)",
        ...
        with st.container():
            ...
            if user_query is not None and user_query.strip() != "":
                ...
                if any(trigger in user_query.lower() for trigger in show_all_triggers):
                    table = "product" # Default table for "show all"
                    lower_query = user_query.lower()
                    if any(keyword in lower_query for keyword in ["supplier", "suppliers", "vendor", "vendors"]):
                        table = "supply"
                    elif any(keyword in lower_query for keyword in ["customer", "customers", "client", "clients"]):
                        table = "customer"
                    elif any(keyword in lower_query for keyword in ["product", "products", "inventory", "stock"]):
                        table = "product"
                    ...
                    table = "product" # Default table for "count"
                    lower_query = user_query.lower()
                    if any(keyword in lower_query for keyword in ["supplier", "suppliers", "vendor", "vendors"]):
                        table = "supply"
                    elif any(keyword in lower_query for keyword in ["customer", "customers", "client", "clients"]):
                        ...
                        if any(phrase in lower_query for phrase in sql_reveal_phrases):
                            ...
                            if schema is None or "Error retrieving schema" in schema:
                                ...
                                prompt_history_for_llm = '\n'.join([
                                    ...
                                    if not (isinstance(m, AIMessage) and "Retail Assistant" in m.content and "Hello!" in m.content) # Exclude initial greeting
                                ])
